...
public class MusicService extends Service implements PlayMusicInterface,
        MediaPlayer.OnCompletionListener {
    public static final String EXTRA_REQUEST_CODE = "REQUEST_CODE";
    public static final int VALUE_NEXT_SONG = 9000;
    public static final int VALUE_PREVIOUS_SONG = 9100;
    public static final int VALUE_PAUSE_SONG = 9200;
    public static final int VALUE_PLAY_SONG = 9300;
    ...
    private PendingIntent mPausePendingIntent, mNextPendingIntent, mPreviousPendingIntent,
            mPlayPendingIntent;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        initLayoutForNotification(R.layout.notification_layout,
                MainActivity.mSongs.get(getCurrentIndex()).getName(), isPlaying());
    ...
    public void create(int index) {
        ...
        int resourceSong = MainActivity.mSongs.get(mCurrentIndex).getResId();
    ...
    public void changeSong(int i) {
        mCurrentIndex += i;
        if (mCurrentIndex >= MainActivity.mSongs.size()) {
            mCurrentIndex = 0;
        } else if (mCurrentIndex < 0) {
            mCurrentIndex = MainActivity.mSongs.size() - NUMBER_1;
        }
    ...
    public void updateNotification() {
        mNotificationLayout.setTextViewText(R.id.text_song_name,
                MainActivity.mSongs.get(mCurrentIndex).getName());
        if (isPlaying()) {
            mNotificationLayout.setViewVisibility(R.id.button_play, View.GONE);
            mNotificationLayout.setViewVisibility(R.id.button_pause, View.VISIBLE);
        } else {
